...
def root():
    return {
        ...
        "usage": 'GET /api/qr?text=https://example.com  with header "X-API-Key: demo-free"',
...
def qr(
    ...
    text: str = Query(..., description="The text or URL to encode"),
    size: int = Query(512, ge=128, le=1024, description="PNG size in pixels"),
):
    try:
        return Response(content=make_qr(text, size), media_type="image/png")
